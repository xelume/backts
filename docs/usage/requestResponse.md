...
status 必须显式指定，接受 200–599，排除 204、205、304；非法值在创建适配器时抛错。省略 `emptyStatus` 时使用严格 JSON 结果契约；无内容可使用直接响应处理器，或按下文配置 `emptyStatus`。此适配器不提供流式响应。HEAD 需要显式注册，执行完整处理和序列化，但不发送正文。
...
仅抛错的异步回调应显式标注 `Promise<结果类型>`，不要依赖推断出的 `Promise<never>`。0.0.36 曾在异常路径触发未处理拒绝并导致进程退出；2026-09-14 的 0.1.1 resultHandler 最小探针在静态编译阶段仍拒绝无结果类型的回调，未解除该限制。已标注回调的错误仍由原有边界处理。

### 可选空结果

`ResultOptions.emptyStatus` 接受 200–599 的整数。设置后，处理器返回 `undefined` 时发送该状态的空响应并跳过序列化；若同时配置 transforms，则该空结果报错。原本有数据的结果经 transforms 变为 `undefined` 时，也发送空响应。省略此选项不会自动启用 204 行为。

Framework 的 `get/post/del` 等方法已配置空结果策略，默认无返回值为 204，通常不需要直接使用这个底层选项。两种响应方式的对照见 [Framework 入门](framework.md#与-core-共用-http-能力)。
