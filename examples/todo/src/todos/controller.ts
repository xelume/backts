...
import { get, post, patch, del, type RoutesOptions } from '@backts/framework';
import {TodoService, TodoInputError, type TodoQuery} from './service';

/** HTTP 输入与业务结果转换；依赖由模块装配，领域错误由控制器边界处理。 */
export function todoRoutes(service: TodoService): RoutesOptions {
  return {
    routes: [
      get('', (context) => {
        const result = service.search(readQuery(context));
        context.header('x-total-count', String(result.total));
        return result.items;
      }),
      get('/:id', (context) => {
        const todo = service.get(readId(context));
      ...
      del('/:id', (context) => {
        const removed = service.remove(readId(context));
