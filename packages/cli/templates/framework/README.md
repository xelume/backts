...
使用 `@backts/framework` 的模块化原生 HTTP 应用，通过 `controller` 和 HTTP 方法入口返回业务结果，无反射或基类要求。
...
入口 `src/main.ts` 创建 AppModule，`src/appModule.ts` 通过 imports 引入 HelloModule 和 HealthModule。编辑 `src/hello.ts` 的 `controller/get` 声明可修改欢迎接口；依赖通过 providers 和 Controller 装配回调解析。模块名称必须唯一，不允许重复导入或循环导入。
